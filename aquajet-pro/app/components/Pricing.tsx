          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
            {plans.map((plan) => (
              ...
              >
                ...
                <p className="text-[12px] font-semibold tracking-[2px] uppercase mb-4" style={{ color: plan.accent }}>
                  {plan.name}
                </p>
                ...
                <div className="flex items-end gap-2 mb-5">
                  ...
                  <span className="text-[14px] text-white/40 pb-2">/ {plan.cadence}</span>
                </div>

                <p className="text-[13px] text-white/40 leading-relaxed mb-6">{plan.description}</p>
                ...
                <ul className="flex flex-col gap-4 mb-8 flex-1">
                  {plan.features.map((feature) => (
                    <li key={feature.title} className="flex items-start gap-3">
                      <svg viewBox="0 0 16 16" fill="none" className="w-4 h-4 shrink-0 mt-[3px]">
                        <circle cx="8" cy="8" r="7" stroke={plan.accent} strokeWidth="1.5" strokeOpacity="0.4" />
                        <path d="M5 8L7 10L11 6" stroke={plan.accent} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                      <div>
                        <p className="text-[13px] font-semibold text-white leading-snug">{feature.title}</p>
                        <p className="text-[12px] text-white/35 leading-relaxed mt-0.5">{feature.desc}</p>
                      </div>
                    </li>
                  ))}
                </ul>
                ...
                <button
                  ...
                  className="w-full py-3.5 rounded-xl font-bold text-[15px] transition-all duration-200 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
                  style={
                    plan.popular
                      ? { background: "#ff00f3", color: "#000000", border: "none" }
                      : { background: "transparent", color: plan.accent, border: `1px solid ${plan.accent}40` }
                  }
                >
